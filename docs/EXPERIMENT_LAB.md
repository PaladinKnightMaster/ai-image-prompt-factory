...
## V3.3 transfer benchmark layer

After EXP-001 through EXP-006 established the initial Isolation Bench, V3.3 adds a second empirical layer under `benchmarks/transfer/`.
...
EXP-008 v1.1.0 is the eighth recorded V3.3 empirical run and the second recorded Transfer Bench result. In `TB-A02`, the broad generic negative stack scored 5.00 on the primary `targeted_failure_mode_control` dimension while the narrower targeted constraint clause scored 4.00; the overall means were 5.0000 and 4.6250 respectively. Semantic compliance and technical-defect cleanliness were unchanged at 5.00, while aesthetic quality was 0.50 lower in the targeted condition. The two lower-scoring targeted outputs contained readable environmental signage/poster text while hand-object interaction remained clean. The run is classified `contradicts`. The related `prompt-targeted-constraints` VisualPattern is downgraded to `mixed` and becomes non-compiler-eligible pending replication, because narrowly scoped constraints can leave adjacent untargeted failure modes exposed.
