...
EXP003_RESULT_REL = Path(
    "experiments/results/EXP-003/"
    "EXP-003-1.0.1-20260911T124433Z-SGB4.result.json"
)


def _exp003_result():
    return json.loads((repo_root() / EXP003_RESULT_REL).read_text(encoding="utf-8"))


def test_exp003_result_schema_validates():
    result = _exp003_result()
    schema = json.loads(
        (repo_root() / "data/schemas/experiment_result.schema.json").read_text(
            encoding="utf-8"
        )
...
def test_exp003_result_aggregates_match_raw_scores():
    result = _exp003_result()
    dimensions = [
        "semantic_compliance",
        "aesthetic_quality",
        "technical_defects",
        "visual_event_coherence",
    ]
...
def test_exp003_result_delta_and_classification():
    result = _exp003_result()
    assert result["primary_dimension"] == "visual_event_coherence"
    assert result["deltas_variant_minus_control"]["visual_event_coherence"] == 1.5
    assert result["deltas_variant_minus_control"]["aesthetic_quality"] == 0.75
    assert result["deltas_variant_minus_control"]["technical_defects"] == -0.25
    assert result["deltas_variant_minus_control"]["overall"] == 0.5
    assert result["evidence_classification"] == "supports"


def test_exp003_definition_is_marked_executed():
    definition = json.loads(
        (repo_root() / "experiments/definitions/EXP-003.json").read_text(
            encoding="utf-8"
        )
    )
    assert definition["version"] == "1.0.1"
    assert definition["status"] == "executed"
    assert definition["latest_result"] == EXP003_RESULT_REL.as_posix()


def test_exp003_pattern_promoted_to_supported_director_rule():
    pattern = get_pattern("narrative-micro-action")
    assert pattern is not None
    assert pattern["validation_status"] == "supported"
    assert pattern["confidence"]["level"] == "supported"
    assert pattern["compiler_usage"]["compiler_eligible"] is True
    assert pattern["compiler_usage"]["director_gate"] is True
    assert "EXP-003-1.0.1-20260911T124433Z-SGB4" in pattern["evidence"]["experiments"]
    assert "controlled_blind_experiment" in pattern["evidence"]["validation_basis"]

EXP005_RESULT_REL = Path(
    "experiments/results/EXP-005/"
    "EXP-005-1.0.1-20260911T175641Z-U8ZL.result.json"
)


def _exp005_result():
    return json.loads((repo_root() / EXP005_RESULT_REL).read_text(encoding="utf-8"))


def test_exp005_result_schema_validates():
    result = _exp005_result()
    schema = json.loads(
        (repo_root() / "data/schemas/experiment_result.schema.json").read_text(
            encoding="utf-8"
        )
...
def test_exp005_result_aggregates_match_raw_scores():
    result = _exp005_result()
    dimensions = [
        "semantic_compliance",
        "aesthetic_quality",
        "technical_defects",
        "spatial_depth_coherence",
    ]
...
def test_exp005_result_delta_and_classification():
    result = _exp005_result()
    assert result["primary_dimension"] == "spatial_depth_coherence"
    assert result["deltas_variant_minus_control"]["spatial_depth_coherence"] == 0.25
...
def test_exp005_definition_is_marked_executed():
    definition = json.loads(
        (repo_root() / "experiments/definitions/EXP-005.json").read_text(
            encoding="utf-8"
        )
    )
    assert definition["version"] == "1.0.1"
    assert definition["status"] == "executed"
    assert definition["latest_result"] == EXP005_RESULT_REL.as_posix()


def test_exp005_pattern_remains_supported_and_compiler_eligible():
    pattern = get_pattern("composition-layered-depth")
    assert pattern is not None
    assert pattern["validation_status"] == "supported"
    assert pattern["confidence"]["level"] == "supported"
    assert pattern["compiler_usage"]["compiler_eligible"] is True
